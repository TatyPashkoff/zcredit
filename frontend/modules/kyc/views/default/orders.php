            ...
            <?php if($model_order) {
                ...
                foreach ($model_order as $item) {
                    ...
                    <tr>
                        ...
                        <td><?=$item->status ?  '<i class="fa fa-check"></i> ' . Yii::t('app','Подтвержден'): '<div class="btn-default btn-small order-confirm" data-id="'.$item->id .'">' .Yii::t('app','Подтвердить').'</div>' ?></td>
                        <td>
                            <div class="row <?=$item->status ? '': 'hidden' ?>">
