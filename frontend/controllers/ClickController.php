...
Class ClickController extends Controller{
    ...
    public function actionPaymentPrepare(){
        ...
        if( !$billing_history = BillingHistory::find()->where(['click_trans'=> $request['click_trans_id']])->one() ) {
            ...
            $billing_history->payment_type = Payment::PAYMENT_TYPE_CLICK;
			$billing_history->click_trans = $request['click_trans_id'];
    ...
    public function actionPaymentComplete(){
        ...
        if(!(
            ...
            return json_encode( array(
                'error' => -8,
                'error_note' => 'Error in request from click - Not all data sent'
            ));
        ...
        if( ! $user = User::find()->where(['id'=> $request['merchant_trans_id'],'role'=>User::ROLE_CLIENT] )->one() ){
        	$this->debug('User does not exist - Action-complete');
            return json_encode( array(
                'error' => -5,
                'error_note' => 'User does not exist'
            ));
		...
		// ПРОВЕРКА ОТ ПОВТОРНОЙ ТРАНЗАКЦИИ
		if(	$clicklog = ClickLog::find()->where(['click_trans_id'=> $request['click_trans_id']] )->one() ){
        	$this->debug('Транзакция уже есть в системе - номер транзакции'.$request['click_trans_id']);
            return json_encode( array(
                'error' => -4,
        ...
        if ((int)$request['action'] == -5017 && $billing_history->state == 1 ) {
        ...
        // check to confirm order paid
        if( (int)$request['error'] == -5017 && $billing_history->state==1 ){
            // return response array-like
            $this->debug('You not enough money- Action-complete');
			$billing_history->state = -1;
            $billing_history->save();
            return json_encode([
                'error' => -9,
        ...
        if($billing_history->state == 1){
            // return response array-like
            $this->debug('Already paid- Action-complete');
            return json_encode([
                'error' => -9,
                'error_note' => 'Already paid'
            ]);
        ...
        if( $request['error'] < 0 ) {
            ...
            $billing_history->state = -1;
            $billing_history->save();
			$user->summ -= $billing_history->summ; // снимает баланс с отмененного заказа у клиента
            $this->debug('The order was cancelled - Status-complete');
            ...
            $clicklog->click_trans_id = $request['click_trans_id'];
            $clicklog->service_id = $request['service_id'];
            $clicklog->merchant_trans_id = $request['merchant_trans_id'];
            $clicklog->amount = $request['amount'];
            $clicklog->action = $request['action'];
            $clicklog->error = $request['error'];
            $clicklog->error_note = $request['error_note'];
            $clicklog->sign_time = $request['sign_time'];
            $clicklog->sign_string = $request['sign_string'];
            $clicklog->click_paydoc_id = $request['click_paydoc_id'];
            $clicklog->merchant_prepare_id = $request['merchant_prepare_id'];
            $clicklog->save();
...
// - ------------------------------------------------------------------------------------------------------------------
    ...
    private function debug($data)
    {
        ...
        $f = fopen($_SERVER['DOCUMENT_ROOT'] .'/click_debug.txt','a');
	...
	private function debug_transactions($data) {
		$f = fopen($_SERVER['DOCUMENT_ROOT'] .'/click_debug_trans.txt','a');
        $data = date('d.m H:i:s') . ':  ' . json_encode($data,256) . PHP_EOL;
        fwrite($f,$data);
        fclose($f);
	}
