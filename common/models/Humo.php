...
/**
 * This is the model class for table "humo".
 *
 * @property integer $id
...
class Humo extends \yii\db\ActiveRecord
{
    ...
    private static $url_test_phone = "http://192.168.35.128:13010"; // test
    ...
    public function rules()
    {
        return [
            [['created_at'], 'integer'],
            [['payment_id','expiry','internal_pan_masked','amount','data'], 'string']
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'payment_id' => Yii::t('app','payment_id'),
            'expiry' => Yii::t('app','expiry'),
            'internal_pan_masked' => Yii::t('app','internal_pan_masked'),
            'data' => Yii::t('app','data'),
    ...
    // scoring
    public function humoScoring($card,$bank_c,$limit){
        $client = $this->backOffice();
        $ex_session = 'ZMARKET-T' . time();
    ...
    // balance
    public function humoBalance($card){
...
XML;
        ...
        //print_r($result->xpath('//SOAP-ENV:Body'));
        $balance = $result->xpath('//balance')[0];
       // $balance = number_format($balance->initialAmount/100, 2, '.', ' ');
    ...
    public function humoSmsBanking($card, $bank_c){
        ...
        $client = $this->humoClient($card, $bank_c);
        $card = $client . '-' . $bank_c;
...
XML;
    ...
    public function HumoDiscard($card_h,$bank_c,$exp,$amount){
...
XML;
        ...
        //return $payment_id;
        if($payment_id){
            $confirm = $this->HumoDiscardConfirm($payment_id);
            return $confirm;
        }
    }
...
XML;
        ...
        $result = simplexml_load_string($result)->xpath('//ebppif1:PaymentResponse')[0];
        $payment_id = strval((int)$result->paymentID);
        $result = $this->HumoRecoFinish($payment_id);
        return $result;
    }
